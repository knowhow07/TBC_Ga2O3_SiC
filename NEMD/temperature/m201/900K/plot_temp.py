...
skip_fraction = 0.5   # use last 50% of data as steady state
...
def main():
    ...
    job_dirs = sorted(
        ...
        if os.path.isdir(d) and d.startswith("job_")
    )
    if not job_dirs:
    ...
    plt.tight_layout()
    outname = f"{compute_file}_profile_all_jobs_{skip_fraction}.png"
