...
X_INTERFACE = 105.848     # your user-defined interface position (Å), e.g. 105
...
FREQ_MAX_THz = 35.0      # plotting max frequency (THz)
...
# IO / MODE SETTINGS
# =========================
IO_MODE = "npy"   # "out" = read velocity.out (and cache), "npy" = read velocity_g2.npy only
...
def main():
    ...
    def do_region(sel, name, color):
        ...
        plt.plot(f, dos_s, linewidth=2, color=color, label=name)

    do_region(sel_left, "SiC(0001)", "tab:green")
    do_region(sel_mid,   "Interface", "tab:blue")
    do_region(sel_right, r"$Ga_2O_3$(-201)", "tab:orange")
    ...
    plt.xlim(0, FREQ_MAX_THz)  # focus on low frequencies relevant for thermal transport
    ...
    plt.savefig(f"pdos_left_mid_right_{WINDOW_W}_{IO_MODE}_extreme.pdf", bbox_inches="tight")
    plt.close()
