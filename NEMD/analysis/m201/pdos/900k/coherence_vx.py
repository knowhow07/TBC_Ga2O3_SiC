...
def build_signals(v_region, mode="mean_vx_demean"):
    ...
    Returns:
      "mean_vx_demean":
          slab-averaged vx(t), after subtracting slab COM vx at each frame
          from every atom in that slab
      "mean_xyz":
          old mode, kept for reference
      "mean_vx":
          old mode, kept for reference
      "sum_ke":
          old mode, kept for reference
    ...
    if mode == "mean_vx_demean":
        # subtract slab COM velocity along x at each frame
        vx = v_region[:, :, 0]                         # (n_frames, n_atoms)
        vx_com = vx.mean(axis=1, keepdims=True)       # (n_frames, 1)
        vx = v_region[:, :, 0]
        vx_com = vx.mean(axis=1, keepdims=True)
        vx_fluc = vx - vx_com

        # RMS fluctuation signal of the slab after COM removal
        sig = {"vx": np.sqrt(np.mean(vx_fluc**2, axis=1))}
...
# here we use only vx and subtract each slab's COM vx at every frame
# before constructing the signal, to suppress rigid drift / low-f noise
