...
def analyze_one_group(group_id, folder, results_dir):
    ...
    # convert to paper unit
    EV_PER_PS_A2_TO_MW_PER_M2 = 1.602176634e7
    # G_like_A_MW = G_like_A * EV_PER_PS_A2_TO_MW_PER_M2
    # Gs_MW = Gs * EV_PER_PS_A2_TO_MW_PER_M2
    # native + paper-unit conductance
    xmin, xmax, xmean = read_group_x_range(model_file, group_id)
    L_group_A = xmax - xmin
    V_group_A3 = A_A2 * L_group_A

    G_native = Jtot / (V_group_A3 * dT)                 # eV ps^-1 THz^-1 Å^-2 K^-1
    Gs = moving_average(G_native, w)

    EV_PER_PS_A2_TO_MW_PER_M2 = 1.602176634e7
    G_native_MW = G_native * EV_PER_PS_A2_TO_MW_PER_M2
    Gs_MW = Gs * EV_PER_PS_A2_TO_MW_PER_M2
    ...
    header = (
        ...
        "G_native_eV_ps^-1_THz^-1_A^-2_K^-1  "
        "G_native_smoothed_eV_ps^-1_THz^-1_A^-2_K^-1  "
        "G_paper_MW_m^-2_K^-1_THz^-1  "
        "G_paper_smoothed_MW_m^-2_K^-1_THz^-1  "
        "cumulative_integral_native"
    )
    np.savetxt(
        data_txt,
        np.column_stack([omega, Jin, Jout, Jtot, G_native, Gs, G_native_MW, Gs_MW, cum]),
        header=header
    )

    # ---- original figures (kept) ----
    plt.figure()
    plt.plot(omega, Gs_MW, label=f"smoothed (w={SMOOTH_W})")
    plt.xlabel("Frequency (THz)")
    plt.ylabel(r"$g(\omega)$ (MW m$^{-2}$ K$^{-1}$ THz$^{-1}$)")
    plt.title(f"SHC normalized by ΔT and group volume (group {group_id})")
...
def make_combined_plots(results, results_dir):
    ...
    # Combined G_like, paper revision
    plt.figure()
    for r in results:
        if r["group_id"] == 3:
            plt.plot(r["omega"], r["Gs_MW"], color="tab:green", label="SiC(0001)")
        elif r["group_id"] == 4:
            plt.plot(r["omega"], r["Gs_MW"], color="tab:blue", label="Interface")
        elif r["group_id"] == 5:
            plt.plot(r["omega"], r["Gs_MW"], color="tab:orange", label=r"Ga$_2$O$_3$($\bar{2}01$)")
        else:
            plt.plot(r["omega"], r["Gs_MW"], label=f"g{r['group_id']}")
    ...
    plt.savefig(f"combined_shc_Glike_S{SMOOTH_W}.pdf", bbox_inches="tight")
    plt.close()
