...
def detect_interface_from_species_change(x: np.ndarray, species: list, Lx: float):
    """
    Detect interface center from the change along +x from Si-rich side to O-rich side.

    Strategy:
    - sort atoms by x
    - find:
        last Si position
        first O position to the right of the Si-rich region
    - interface center = midpoint between those two positions

    Returns:
        x_center, x_last_si, x_first_o
    """
    sp = np.array(species)
    order = np.argsort(x)
    xs = x[order]
    ...
    if not np.any(si_mask):
        raise RuntimeError("No Si atoms found; cannot detect interface from Si -> O change.")
    if not np.any(o_mask):
        raise RuntimeError("No O atoms found; cannot detect interface from Si -> O change.")

    x_last_si = np.max(xs[si_mask])

    # first O atom to the right of the last-Si position
    o_right = xs[o_mask & (xs >= x_last_si - EPS)]
    if len(o_right) == 0:
        raise RuntimeError(
            "Could not find O atoms to the right of the Si side. "
            "Check x direction, wrapping, or species labels."
        )

    x_first_o = np.min(o_right)
    x_center = 0.5 * (x_last_si + x_first_o)

    if not (0.0 - 1e-6 <= x_center <= Lx + 1e-6):
        raise RuntimeError(
            f"Detected interface center x={x_center:.3f} Å is outside box [0, {Lx:.3f}] Å."
        )

    return x_center, x_last_si, x_first_o
...
def main():
    ...
    if not os.path.exists(original):
        ...
        log(f"Renamed {infile} -> {original} (saved original)")
    ...
    # Choose interface center from species change along x: Si -> O
    x_center, x_last_si, x_first_o = detect_interface_from_species_change(x, species, Lx)
    ...
    log(f"Loaded {N} atoms from {original}")
    log(f"Box Lx (from Lattice) = {Lx:.3f} Å")
    log(f"Detected interface by species change along x")
    log(f"  last Si x   = {x_last_si:.3f} Å")
    log(f"  first O x   = {x_first_o:.3f} Å")
    log(f"  interface x = {x_center:.3f} Å")
    log(f"Requested widths: left={SIDE_W:.3f}, interface={INTERFACE_W:.3f}, right={SIDE_W:.3f} (Å)")
    log(f"WALL_THICK_BASE={WALL_THICK_BASE:.3f} Å each end -> min_needed={min_needed:.3f} Å")
