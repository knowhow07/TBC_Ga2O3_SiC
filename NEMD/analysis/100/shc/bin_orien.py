...
TEMP_PATHS = {
    100: "./fix/results",
    -201: "<PATH_TO_DATA>/qe_jobs/nep_training/NEMD/0217/post/210-300k/20+10/shc/300k/results",
}
...
def main():
    ...
    temps = [100, -201]
    temps = [T for T in temps if T in loaded]
    ref_T = temps[0]
    ...
    for k, T in enumerate(temps):
        ...
        plt.bar(xpos + shift, vals, width=bar_w,
        label=r"$Ga_2O_3$($\bar{2}$01)/SiC(0001)" if T == -201 else r"$Ga_2O_3$(100)/SiC(0001)")

    plt.xticks(xpos, labels)
    plt.xlabel("Frequency bin (THz)")
    plt.ylabel(r"Integrated $g(\omega)$ (MW m$^{-2}$ K$^{-1}$)",fontsize=20)
