...
# USER SETTINGS
# =========================
path = "<PATH_TO_DATA>/qe_jobs/nep_training/NEMD/0217/post/MFP/Ga2O3/201/hnemd"
...
def read_kappa_running(kappa_file: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
    omega_raw, kappa_omega_raw

    Notes:
    - col0 in shc.out is treated as raw omega from GPUMD spectral output
    - for comparison with PDOS, usually plot/store freq = omega / (2*pi)
    - kappa_omega_raw is still the spectral quantity on the raw omega grid
    """
...
plt.tight_layout()
fig_running = f"{path}/hnemd_running_kappa_avg_{DIRECTION}.png"
plt.savefig(fig_running, dpi=300)
# plt.savefig(f"{fig_running.replace('.png', '')}.pdf", bbox_inches="tight")
...
plt.plot(freq_ref[plot_mask_w], spec_avg[plot_mask_w], linewidth=3, label=f"kappa_{DIRECTION}(ω) avg")
...
plt.title(r"HNEMD spectral thermal conductivity, $Ga_2O_3$(-201)", fontsize=22)
plt.legend()
plt.tight_layout()

fig_spec = f"{path}/hnemd_spectral_kappa_avg_{DIRECTION}.png"
plt.savefig(fig_spec, dpi=300)
plt.savefig(f"{fig_spec.replace('.png', '')}.pdf", bbox_inches="tight")
...
fig_cum = f"{path}/hnemd_cumulative_kappa_avg_{DIRECTION}.png"
