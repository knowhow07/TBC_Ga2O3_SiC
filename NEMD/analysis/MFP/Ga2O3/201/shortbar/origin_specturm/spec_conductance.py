...
# USER KNOBS
# =========================
PATH_ROOT   = "<PATH_TO_DATA>/qe_jobs/nep_training/NEMD/0217/post/210-300k/pure/Ga2O3"
...
OUT_PREFIX = "bulk_ga2o3_shortbar"

# native -> SI for later use elsewhere if needed:
...
def main():
    ...
    for job in job_dirs:
        ...
        # ---- Native quantities ----
        # J_area: eV ps^-1 THz^-1 Å^-1
        J_area = Jtot / area_A2

        # G_native: eV ps^-1 THz^-1 Å^-2 K^-1
        ...
        print(f"L_SHC = {L_shc_A:.6f} Å")
        print(f"V_SHC = {V_shc_A3:.6f} Å^3")
    ...
    # ---- Stack and average ----
    J_area_arr = np.vstack(J_area_runs)
    ...
    Gs = moving_average(G_native_avg, SMOOTH_W)

    # ---- Cumulative integral (plotting only) ----
    cum_G = np.zeros_like(Gs)
    if len(freq_plot) > 1:
        cum_G[1:] = np.cumsum(0.5 * (Gs[1:] + Gs[:-1]) * np.diff(freq_plot))
    ...
    print(f"Mean L_SHC over jobs: {np.mean(L_shc_list):.6f} Å")
    print(f"Mean V_SHC over jobs: {np.mean(V_shc_list):.6f} Å^3")
    ...
    # ---- Save averaged native spectral conductance ----
    out_avg_native = np.column_stack([
        freq_plot,
        G_native_avg,
        G_native_stderr,
    ])
    out_avg_native_file = os.path.join( f"{OUT_PREFIX}_Gomega_avg_native.txt")
    np.savetxt(
        out_avg_native_file,
        out_avg_native,
        header=(
    ...
    print(f"\nSaved: {out_avg_native_file}")

    # ---- Save per-run native conductance ----
    out_runs_native = np.column_stack([freq_plot] + [G_native_arr[i] for i in range(G_native_arr.shape[0])])
    run_cols_native = "  ".join(
        [f"G_native_run{i+1:02d}_eV_ps^-1_THz^-1_A^-2_K^-1" for i in range(G_native_arr.shape[0])]
    )
    out_runs_native_file = os.path.join( f"{OUT_PREFIX}_Gomega_all_runs_native.txt")
    np.savetxt(
    ...
    # ---- Save cumulative native conductance ----
    cum_native_file = os.path.join(f"{OUT_PREFIX}_Gomega_cumulative_native.txt")
    np.savetxt(
        cum_native_file,
        np.column_stack([freq_plot, cum_G]),
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(f"mean_L_SHC_A = {np.mean(L_shc_list):.10f}\n")
        f.write(f"mean_V_SHC_A3 = {np.mean(V_shc_list):.10f}\n")
    ...
    plt.xlabel(xlab)
    plt.ylabel(r"Spectral heat current / area (native units / Å$^2$)")
    plt.title(f"Bulk Ga2O3 short-bar SHC / area (group {SHC_GROUP_ID})")
    plt.grid(True, linestyle="--", alpha=0.35)
    plt.legend()
    plt.tight_layout()
    fig1 = os.path.join(f"{OUT_PREFIX}_Jarea_avg.png")
    plt.savefig(fig1, dpi=300)
    print(f"Saved: {fig1}")

    # ---- Plot native spectral conductance ----
    plt.figure(figsize=(10, 6))
    ...
    plt.plot(freq_plot[mask_w], Gs[mask_w], color="C1", linewidth=2, label=f"avg smooth (w={SMOOTH_W})")
    ...
    plt.title(f"Bulk Ga2O3 short-bar spectral conductance (native, group {SHC_GROUP_ID})")
    plt.grid(True, linestyle="--", alpha=0.35)
    plt.legend()
    plt.tight_layout()
    fig2 = os.path.join(f"{OUT_PREFIX}_Gomega_avg_native.png")
    plt.savefig(fig2, dpi=300)
    print(f"Saved: {fig2}")

    # ---- Plot cumulative native spectral conductance ----
    plt.figure(figsize=(10, 6))
    plt.plot(freq_plot[mask_w], cum_G[mask_w], color="C0", linewidth=2)
    plt.xlabel(xlab)
    plt.ylabel(r"Cumulative $\int G(\omega)\,d\omega$ (eV ps$^{-1}$ Å$^{-2}$ K$^{-1}$)")
    plt.title("Cumulative spectral conductance (native)")
    plt.grid(True, linestyle="--", alpha=0.35)
    plt.tight_layout()
    fig3 = os.path.join( f"{OUT_PREFIX}_Gomega_cumulative_native.png")
