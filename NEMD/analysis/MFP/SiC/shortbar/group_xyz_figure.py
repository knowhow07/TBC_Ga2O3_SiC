...
path = ""
infile  = os.path.join(path, "rotated_longx_SiC_out.xyz")   # input from your rotation step
outfile = os.path.join(path, "model.xyz")           # GPUMD-ready output
...
N_inter_bins = 70     # number of temperature bins between hot and cold
plot_file    = os.path.join(path,"group_ranges_x.png")
# -------------------------------------------
...
# =======================
# Visualize x-range of each group
# =======================
group_ids  = []
group_xmin = []
group_xmax = []
...
for g in unique_groups:
    if g == 0:
        # split walls into left and right pieces so they
        # don't appear as one continuous bar
        maskL = (groups == 0) & (x <= x_fixL_hi)
        maskR = (groups == 0) & (x >= x_fixR_lo)
        if np.any(maskL):
            group_ids.append(0)
            group_xmin.append(x[maskL].min())
            group_xmax.append(x[maskL].max())
        if np.any(maskR):
            group_ids.append(0)
            group_xmin.append(x[maskR].min())
            group_xmax.append(x[maskR].max())
    else:
        mask = (groups == g)
        group_ids.append(g)
        group_xmin.append(x[mask].min())
        group_xmax.append(x[mask].max())

group_ids  = np.array(group_ids)
group_xmin = np.array(group_xmin)
group_xmax = np.array(group_xmax)

plt.figure(figsize=(8, 6))
for xmin, xmax, gid in zip(group_xmin, group_xmax, group_ids):
    plt.hlines(y=gid, xmin=xmin, xmax=xmax, linewidth=3)

plt.xlabel("Position along x (Å)")
plt.ylabel("Group ID")
plt.title("Group ranges along x-direction")
...
print(f"✔ Saved group x-range plot to: {plot_file}")
