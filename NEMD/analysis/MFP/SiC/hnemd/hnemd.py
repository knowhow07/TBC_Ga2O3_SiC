...
# USER SETTINGS
# =========================
path = "<PATH_TO_DATA>/qe_jobs/nep_training/NEMD/0217/post/MFP"
...
def read_kappa_running(kappa_file: str) -> tuple[np.ndarray, np.ndarray]:
    """
    Read GPUMD kappa.out.
    For the driven diagonal component in 3D, use col0 + col1.
    Returns:
        t_ps, k_running
    """
    ...
    if data.shape[1] < 2:
        raise ValueError(f"{kappa_file} has fewer than 2 columns.")
...
for kappa in kappa_files:
    t_ps, k_running = read_kappa_running(kappa)
...
for shc in shc_files:
    omega_thz, kappa_omega = read_spectral_kappa(
        shc_file=shc,
        num_omega=NUM_OMEGA,
    ...
    if omega_ref is None:
        ...
        if len(omega_thz) != len(omega_ref) or not np.allclose(omega_thz, omega_ref):
            raise ValueError(f"Frequency grid mismatch in {shc}")

    spec_list.append(kappa_omega)
...
if MAX_W_PLOT is not None:
    plot_mask_w = omega_ref <= MAX_W_PLOT
else:
    plot_mask_w = np.ones_like(omega_ref, dtype=bool)
...
out_spec = np.column_stack([omega_ref, spec_avg])
np.savetxt(
    out_spec_file,
    out_spec,
    header=f"omega_THz  kappa_{DIRECTION}_avg_Wm-1K-1THz-1\nAveraged over {n_runs_spec} runs",
    fmt="%.8e"
)
...
out_spec_stat = np.column_stack([omega_ref, spec_avg, spec_std, spec_stderr])
np.savetxt(
    out_spec_stat_file,
    out_spec_stat,
    header=(
        f"omega_THz  kappa_{DIRECTION}_avg_Wm-1K-1THz-1  "
...
for i in range(n_runs_spec):
    plt.plot(omega_ref[plot_mask_w], spec_arr[i, plot_mask_w], color="gray", alpha=0.2, linewidth=1)

plt.plot(omega_ref[plot_mask_w], spec_avg[plot_mask_w], linewidth=3, label=f"kappa_{DIRECTION}(ω) avg")

if n_runs_spec > 1:
    plt.fill_between(
        omega_ref[plot_mask_w],
...
plt.title(f"HNEMD spectral thermal conductivity ({DIRECTION})")
plt.legend()
plt.tight_layout()

fig_spec = f"{path}/hnemd_spectral_kappa_avg.png"
