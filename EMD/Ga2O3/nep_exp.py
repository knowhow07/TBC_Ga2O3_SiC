...
def load_nep_points_with_errorbars(base_dir: str) -> Tuple[np.ndarray, Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    ...
    Ts: List[float] = []
    means_by_dir = {d: [] for d in DIRECTIONS}
    errs_by_dir = {d: [] for d in DIRECTIONS}
    ...
    for folder in folders:
        ...
        for d in DIRECTIONS:
            ...
            means_by_dir[d].append(mean)
            errs_by_dir[d].append(stderr)

    if not Ts:
        return np.array([]), {d: np.array([]) for d in DIRECTIONS}, {d: np.array([]) for d in DIRECTIONS}

    order = np.argsort(Ts)
    T_arr = np.array(Ts, dtype=float)[order]
    mean_arrs = {d: np.array(means_by_dir[d], dtype=float)[order] for d in DIRECTIONS}
    err_arrs = {d: np.array(errs_by_dir[d], dtype=float)[order] for d in DIRECTIONS}
...
def main():
    ...
    for ds_name, ds in exp_data.items():
        for d in DIRECTIONS:
            ...
            t = [p[0] for p in pts]
            k = [p[1] for p in pts]
            # --- Replace your experimental scatter call with this ---
            plt.scatter(t, k, marker="o", s=MARKER_SIZE_EXP, label=f"{ds_name} {d}")


    # NEP with error bars (stderr)
    if nep_T.size > 0:
        for d in DIRECTIONS:
            # --- Replace your NEP errorbar call with this ---
            plt.errorbar(
                nep_T,
                nep_mean[d],
                yerr=nep_err[d],
