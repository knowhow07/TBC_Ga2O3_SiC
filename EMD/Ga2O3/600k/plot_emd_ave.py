...
MAX_T_PLOT = 200      # e.g. 50.0 for 0–50 ps, or None for all
...
plt.legend()
plt.tight_layout()
plt.savefig("emd_hfacf_normalized_avg.png", dpi=300)
print("Saved HFACF figure: emd_hfacf_normalized_avg.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("emd_hfacf_running_integral.png", dpi=300)
print("Saved HFACF running-integral figure: emd_hfacf_running_integral.png")
...
i0, i1 = find_plateau_indices(t_ref, kiso_avg,
                              min_fraction=0.3,
                              window_ps=50.0,
                              slope_threshold=5e-4)

t_start, t_end = t_ref[i0], t_ref[i1 - 1]
print(f"Plateau window: {t_start:.2f} – {t_end:.2f} ps "
      f"({i1 - i0} points)")
...
def plateau_stats(arr, i_start, i_end):
    """
...
for name, arr in [
    ...
    mean, std, stderr = plateau_stats(arr, i0, i1)
    results[name] = (mean, std, stderr)
    print(f"{name:6s}: mean = {mean:8.3f} W/mK, "
          f"std = {std:8.3f}, stderr = {stderr:8.3f}")
...
print("Saved plateau statistics to: kappa_plateau_stats.txt")
